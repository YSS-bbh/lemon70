...
def automatic(excel, sheet, log=None):
    res_excel = excel_master_data(excel, sheet)
    a = 0
    if not isinstance(res_excel, str):
        ...
        for excel in res_excel:
            ...
            if eval(register_expected) == res_expected:
                print('通过')
                cg = update_excel_expected(excel, sheet, excel['case_id'] + 1, 8, '通过')
                consequence = {'sheet': sheet, 'case_id': excel['case_id'], 'result': cg}
                list_1.append(consequence)
            else:
                print('不通过')
                sb = update_excel_expected(excel, sheet, excel['case_id'] + 1, 8, '不通过')
